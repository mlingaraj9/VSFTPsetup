...
to one or more HTTP endpoint(s) as JSON.
...
| `xml_forwarder.py` | The forwarder daemon. Polls the upload dir, converts Vigilant XML to JSON, POSTs to all endpoints, moves sent files to `_processed/`, deletes them after a grace period. |
...
10. Creates `/var/log/xml_forwarder.log` owned by `cameraftp`
...
| `log_file` | Forwarder log path | `/var/log/xml_forwarder.log` |
| `endpoints` | List of destination URLs; a file is only "done" when ALL return 2xx | — |
| `camera_name_map` | Map camera name in XML → name to send to the endpoint (unmapped names pass through) | `{}` |
...
sudo tail -f /var/log/xml_forwarder.log # parse/send/move/delete activity
...
State is tracked in `upload/.xml_forwarder_state.json`. If that file is
